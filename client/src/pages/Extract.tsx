            ...
            <>
              ...
              {!thumbLoading && thumbnails.length > 0 && (
                <>
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-xs text-gray-500">
                      {selected.size > 0
                        ...
                        : `${pageCount} page${pageCount !== 1 ? 's' : ''} — click to select`}
                    </span>
                    <button
                      onClick={toggleAll}
                  ...
                  <div className="grid grid-cols-4 sm:grid-cols-6 gap-2">
                    {thumbnails.map((src, i) => (
                      ...
                      >
                        ...
                        <div className={`absolute inset-0 transition-colors ${selected.has(i) ? 'bg-indigo-500/10' : ''}`} />
                  ...
                  {selected.size > 0 && (
                    <div className="mt-4 px-3 py-2 bg-indigo-50 border border-indigo-100 rounded-lg text-xs text-indigo-700">
                      Pages selected:{' '}
        ...
        <div className="flex flex-col items-center gap-4 py-10 text-center">
          {isProcessing && (
            <>
              ...
              <p className="text-sm text-gray-500">Extracting pages…</p>
            </>
          )}
          ...
          {isDone && (
            <>
              ...
              <p className="text-sm font-medium text-gray-900">Pages extracted!</p>
              <div className="flex gap-3">
                <a href={jobsApi.downloadUrl(jobId)}>
                  <Button>Download PDF</Button>
                </a>
                <Button variant="ghost" onClick={reset}>Extract from another</Button>
              </div>
            </>
          )}
